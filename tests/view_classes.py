...
class JSONifyTestView(FlaskView):
    ...
    def not_found(self):
        return jsonify(dict(success=False)), 404

    @route("/custom-header", methods=["GET"])
    def custom_header(self):
        return jsonify(dict(success=True)), 418, {"X-TEAPOT": "1"}
...
class DecoratedListFunctionAttributesView(FlaskView):
    ...
    decorators = [make_italics_decorator, eggs_attribute_decorator("scrambled")]
